...
public class RepeatService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        //thread.start();
        Toast.makeText(getApplicationContext(),"RepeatService.class 시작 되었습니다!",Toast.LENGTH_SHORT).show();
        mTask = new TimerTask() {
            @Override
            public void run() {
                Intent intent = new Intent(RepeatService.this, AlarmActivity.class);
                startActivity(intent);
                //Toast.makeText(getApplicationContext(), "복약하세요", Toast.LENGTH_LONG).show();
            }
        };
        mTimer = new Timer(true);
        mTimer.schedule(mTask, 1000, 9000);
        return START_STICKY;
    }
